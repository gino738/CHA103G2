package com.cha103g2.members.dao;
...
public class MembersJDBCDAO implements MembersDAO_interface {
	...
	private static final String INSERT = "INSERT INTO members(mem_no,mem_status,mem_name,mem_mail,mem_account,mem_pass,mem_gender,mem_id,mem_bir, mem_phone,mem_address,mem_date) VALUES (?, ?, ?, ?, ?, ?, ? ,?, ?, ?,?, ?)";
	private static final String UPDATE_ALL = "UPDATE members SET mem_status = ?, mem_name = ?, mem_mail = ?, mem_account = ?, mem_pass = ?, mem_gender = ?, mem_id = ?, mem_bir = ? , mem_phone = ?,mem_address = ? WHERE mem_no = ?";
	private static final String UPDATE_BACK_STATUS = "UPDATE members SET mem_status = ? WHERE mem_no = ?";
	private static final String UPDATE_FRONT = "UPDATE members SET mem_name = ?, mem_mail = ?, mem_gender = ?, mem_id = ?, mem_bir = ? , mem_phone = ?,mem_address = ? WHERE mem_no = ?";
...
//	private static final String UPDATE_MEMCAPTCHA = "UPDATE members SET mem_captcha = ? WHERE mem_no = ?";
	...
	public void insert(MembersVO mVO) {
		...
		try {
			...
			pstmt.setTimestamp(12, mVO.getMemdate());
	...
	public void updateFront(MembersVO mVO) {
		...
		try {
			...
			pstmt.setInt(3, mVO.getMemgender());
			pstmt.setString(4, mVO.getMemid());
			pstmt.setDate(5, mVO.getMembir());
			pstmt.setString(6, mVO.getMemphone());
			pstmt.setString(7, mVO.getMemaddress());
			pstmt.setInt(8, mVO.getMemno()); 
