...
import com.cha103g2.members.dao.*;
import com.cha103g2.members.service.*;
...
//					System.out.print((char)Arr[i]);
	...
	protected int allowUser(String memaccount,String mempass) {
	...
	//========註冊驗證錯誤
	protected int checkUser(String memaccount,String captcha) {
        ...
        if (membersVO.getMemaccount().equals(memaccount) && membersVO.getMemcaptcha().equals(captcha)) {
        	System.out.println("成功登入");
        	return 3;
        }else {
...
//		if("checkIfAccountExist".equals(action)) {
//			
//			JSONObject output = new JSONObject();
//			String inputAccount=req.getParameter("inputaccount");
//			MembersService membersSvc=new MembersService();
//			MembersVO membersVO=null;
//			membersVO=membersSvc.get_one_bymemaccount(inputAccount);
//			
//			try {
//		        output.put("available", ((membersVO==null)?"Y":"N"));
...
//		}
		...
		if("logout".equals(action)) {
			...
			session.removeAttribute("memno");//why是移除會員編號?
		...
		if("checkMem".equals(action)) {
			...
			if(checkUser(memaccount,memcaptcha)==1) {//註冊驗證?
				//登入不成功
				System.out.println("沒有此帳號");
				String URL=req.getContextPath()+"/front-end/member/MemberVerify.jsp?error=noAccount";
				res.sendRedirect(URL);
				return;
			}else if(checkUser(memaccount,memcaptcha)==4) {
				//登入不成功
				System.out.println("驗證碼錯誤");
...
//				membersSvc.update_memverify("1",membersVO.getMemno());//無此功能
//				HttpSession session=req.getSession();
//				session.setAttribute("memverify", memberVO.getMemno());
				res.sendRedirect(req.getContextPath()+"/front-end/member/MemberCaptchaVerify.jsp?error=success"); 
			}
...
//				} 
				...
				System.out.println("memno存session");
...
//			
//			try {
//				Integer memno=req.getParameter("memno");
//				MembersService membersSvc=new MembersService();
//				MembersVO membersVO=new MembersVO();
//				membersVO=membersSvc.getOneMember(memno);
//				
//			
//				
