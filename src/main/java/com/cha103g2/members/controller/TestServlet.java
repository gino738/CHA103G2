...
public class TestServlet<TestService> extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if ("getOneBymemno".equals(action)) { // 來自select_page.jsp的請求
			...
			req.setAttribute("errorMsgs", errorMsgs);
				...
				req.setAttribute("memsVO", MembersVO); // // 資料庫取出的empVO物件,存入req
...
//		if ("updateBackStatus".equals(action)) { // 來自listAllEmp.jsp的請求
//
//			List<String> errorMsgs = new LinkedList<String>();
//			// Store this set in the request scope, in case we need to
//			// send the ErrorPage view.
...
//				String memaccountReg;
//				if (memaccount == null || memaccount.trim().length() == 0) {
//					errorMsgs.add("會員帳號: 請勿空白");
//				} else if(!memaccount.trim().matches(memnameReg)) { //以下練習正則(規)表示式(regular-expression)
//					errorMsgs.add("會員姓名: 只能是中、英文字母、數字和_ , 且長度必需在2到10之間");
//				}
//				
//				
